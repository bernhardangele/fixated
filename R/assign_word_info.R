...
#' a data frame of fixations based on word boundary positions.  A
...
#'   columns `word_id` (integer), `word` (character), and
#'   `x_end` (numeric, pixels).  Typically the output of
#'   [read_roi()] or a post-processed `word_boundaries` element
...
#'   value receive `word_id = -1`.  Defaults to `125`.
#'
#' @return A copy of `fixations` with two new columns added:
#'   \describe{
#'     \item{`word_id`}{Integer word number, or `-1` for fixations before
...
assign_word_info <- function(fixations,
  ...
  stopifnot(all(c("word_id", "word", "x_end") %in%
                  names(word_boundaries)))

  fx         <- fixations$x
  boundaries <- sort(word_boundaries$x_end)
  ...
  } else if ("trial" %in% names(word_boundaries)) {
    out$trial_nr <- word_boundaries$trial[[1L]]
  }
